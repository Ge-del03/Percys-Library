...
namespace ComicReader.ViewModels
{
    ...
    private readonly ILogService _log;
        ...
        private async void LoadThumbnailsAsync(CancellationToken ct)
        {
            ...
            try
            {
                var thumbnailTasks = ComicLoader.Pages.Select(async (page, index) =>
                {
                    ...
                    var bookmarks = _bookmarkService.GetBookmarksForComic(ComicLoader?.FilePath ?? string.Empty)
                    ...
                    var item = new ThumbnailItem
                    {
                        PageIndex = index,
                        PageNumber = index + 1,
                        Thumbnail = thumbnail ?? new BitmapImage(),
        ...
        private async Task<BitmapImage?> GetOrCreateThumbnailAsync(int pageIndex, int size, CancellationToken ct)
        {
            ...
            {
                ct.ThrowIfCancellationRequested();
                var fileKey = ComicLoader?.FilePath ?? string.Empty;
                var cacheKey = $"thumb_{fileKey}_{pageIndex}_{size}";
        ...
        public void AddBookmarkToCurrentPage(string note = "")
        {
            if (ComicLoader?.Pages != null && CurrentPageIndex >= 0 && CurrentPageIndex < ComicLoader.Pages.Count)
            {
                ...
                var thumbnailImage = currentThumbnail?.Thumbnail ?? new BitmapImage();
                var loader = ComicLoader!;
                _bookmarkService.AddBookmark(loader.FilePath ?? string.Empty, loader.ComicTitle ?? string.Empty, CurrentPageIndex + 1, thumbnailImage, note);
